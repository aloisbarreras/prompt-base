...
describe('utils.actions', function() {
  it('should decorate actions on keypress events', () => {
    ...
    assert.equal(action({ name: 'tab' }), 'next');
  ...
  it('should decorate actions for ctrl+', () => {
    ...
    assert.equal(action({ name: 'c', ctrl: true }), 'abort');
    assert.equal(action({ name: 'd', ctrl: true }), 'abort');
  ...
  it('should decorate actions for shift+', () => {
    ...
    assert.equal(action({ name: 'tab', shift: true }), 'prev');
    assert.equal(action({ name: 'up', shift: true }), 'shiftUp');
  });

  it('should handle numbers', () => {
    assert.equal(actions('0', { name: '0' }).name, 'number');
    assert.equal(actions('1', { name: '1' }).name, 'number');
    assert.equal(actions('2', { name: '2' }).name, 'number');
    assert.equal(actions('3', { name: '3' }).name, 'number');
    assert.equal(actions('4', { name: '4' }).name, 'number');
    assert.equal(actions('5', { name: '5' }).name, 'number');
    assert.equal(actions('6', { name: '6' }).name, 'number');
    assert.equal(actions('7', { name: '7' }).name, 'number');
    assert.equal(actions('8', { name: '8' }).name, 'number');
    assert.equal(actions('9', { name: '9' }).name, 'number');
  });

  it('should handle shift+number', () => {
    assert.equal(actions('!', { name: '!', shift: true }).name, '!');
    assert.equal(actions('@', { name: '@', shift: true }).name, '@');
    assert.equal(actions('#', { name: '#', shift: true }).name, '#');
    assert.equal(actions('$', { name: '$', shift: true }).name, '$');
    assert.equal(actions('%', { name: '%', shift: true }).name, '%');
